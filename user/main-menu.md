...
It works anywhere in the Steam interface, so this is how you get into Junk Store without
...
any machine without a Deck's controls. These come from Steam rather than Junk Store, so
...
**Ctrl+3 is the one you want here.** Junk Store's own keyboard shortcuts are separate and
listed in [File Manager reference](file-manager-reference.md#getting-here-from-anywhere).

## Signing in to Junk Store

Junk Store itself needs an account. It's the one you made when you bought it, and it's what
...
Enter the details you used when you bought Junk Store and it goes straight on to the menu.
...
address, and there's a **Logout** button beside it. Logging out asks you to confirm, and
...
or if you need to sign in as a different account. If Junk Store simply stops believing you
...
into Junk Store; it's extensions putting their own actions somewhere you can reach them. If
...
**News** is the Junk Store blog, pulled in as a feed so you can read it on the device
...
**This is a separate thing from the sign-in above.** That one is your Junk Store account.
This one is your account with **Epic, GOG or whichever store an extension is for**, and the
two are unrelated: signing in to Junk Store doesn't sign you in to any store, and logging
...
in there doesn't sign in the client Junk Store installs games with, which catches people
out regularly. See
...
**Junk Store never sees your password.** It puts the store's own login page on screen and
you sign in with them directly, the same as you would on their website.

Getting that page onto a Deck in Game Mode is the awkward part. Junk Store handles it by
...
- **Your credentials go to the store, not to Junk Store.** There's nowhere for them to be
...
**This needs the clients Junk Store supplies.** It doesn't work with the older flatpak
...
Pick one and that account becomes the one Junk Store acts as. Nothing is signed out in the
...
Store supplies, so your progress lives on the Deck rather than being synced to the store.
...
and downloads, but the grid keeps showing everything Junk Store knows about from that
